...
# Fix generated Cargo.toml for project conventions
_fix-workspace-refs decoder_name:
    #!/bin/bash
    echo "Fixing Cargo.toml for {{decoder_name}}..."

    # Update edition to 2024 (CLI generates 2021)
    just _sed 's/edition = "2021"/edition = "2024"/' ./dist/{{decoder_name}}/Cargo.toml

    # Remove [workspace] section (conflicts with parent workspace)
    just _sed '/^\[workspace\]$/d' ./dist/{{decoder_name}}/Cargo.toml

    # Add serde feature to solana-pubkey for JSON serialization support
    just _sed 's/solana-pubkey = { version = "\^3.0.0", features = \["borsh"\] }/solana-pubkey = { version = "3.0.0", features = ["borsh", "serde"] }/' ./dist/{{decoder_name}}/Cargo.toml
...
# Player Profile
generate-player-profile: (generate "player-profile")
build-player-profile: (build "player-profile")
clean-player-profile: (clean "player-profile")
apply-patches-player-profile: (apply-patches "player-profile")
...
apply-patches-points: (apply-patches "points")
